...
class Eshop {
    ...
    public function proccessItem($userid,$ItemNum){
        ...
        $this->_epoints = $accountInfo['UserPoint'];
    ...
    public function GetEP($id) {
        ...
        $result = $this->db3->query_fetch_single("SELECT UserPoint FROM UserInfo WHERE UserNum = ?", array($id));
    ...
    private function subractCharacterPoint($id,$itemprice){
        ...
        $result = $this->db3->query("UPDATE UserInfo SET UserPoint = UserPoint-? WHERE UserNum = ?", array($itemprice, $id));
        if($result) return true;
        return;
    }
            ...
            while(!feof($fp)){
                ...
                foreach($line as $itm){
                    ...
                    if ($find == "IN_") {
                        ...
                        $so = explode("\t",$itm);
                        //var_dump($so[1]);
                        $result = $this->db2->query("INSERT INTO ShopItemMap (ItemMain,ItemSub,ItemName,ItemPrice,Itemstock,Category,PremiumItem) VALUES (?,?,?,?,?,?,?)", array($mid,$sid,$so[1],0,0,0,1));
