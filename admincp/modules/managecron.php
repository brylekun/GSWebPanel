 ...
 */
?>
<section class="content-header"><h1> Cron Job Manager </h1></section>
...
if(check_value($_REQUEST['cache']) && $_REQUEST['cache'] == 1) {
	$cacheDATA = BuildCacheData(glenox::DB(config('SQL_RANPANEL'))->query_fetch("SELECT * FROM cron"));
...
if(check_value($_REQUEST['reset']) && $_REQUEST['reset'] == 1) {
	$resetCrons = glenox::DB(config('SQL_RANPANEL'))->query("UPDATE cron SET cron_last_run = NULL");
	if($resetCrons) {
		$cacheDATA = BuildCacheData(glenox::DB(config('SQL_RANPANEL'))->query_fetch("SELECT * FROM cron"));
		UpdateCache('cron.cache',$cacheDATA);
...
$cronJobs = glenox::DB(config('SQL_RANPANEL'))->query_fetch("SELECT * FROM cron ORDER BY cron_id ASC");
