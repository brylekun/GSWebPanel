<section class="content-header"><h1> VP-Shop Settings </h1></section>
...
if(check_value($_POST['newitem_submit'])) {
	try {
		...
		message('success', 'Success insert new item! wait 2 second to refresh');
		redirect(2,'admincp/index.php?module=modules_manager&config=vshop',2);
	} catch(Exception $ex) {
...
if(check_value($_POST['update_submit'])) {
	...
	message('success', 'Changes successfully saved! wait 2 second to refresh');
	redirect(2,'admincp/index.php?module=modules_manager&config=vshop',2);
	} catch(Exception $ex) {
			message('error', $ex->getMessage());
	}
...
if(check_value($_GET['delete'])) {
	try {
		...
		unset($_POST['ItemNum']);
		message('success', 'Item has been delete! wait 2 second to refresh');
		redirect(2,'admincp/index.php?module=modules_manager&config=vshop',2);
	} catch(Exception $ex) {
			message('error', $ex->getMessage());
	}
			...
			foreach($result as $id => $IShop) {
				...
				echo '<tr>';
						...
						echo '<td class="text-center" style="vertical-align:middle;"><a href="?module=modules_manager&config=vshop&delete='.$IShop['ProductNum'].'" onclick="return confirm(\'Are you sure! you want to delete this item?\')" class="btn btn-danger btn-xs"><span class="fa fa-times" aria-hidden="true"></span></a></td>';
						// item Main / Sub
